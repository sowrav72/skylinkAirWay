  ...
  const confirmBooking = async () => {
    ...
    try {
      const res  = await fetch(`${API}/bookings/`, {
        ...
        body: JSON.stringify({ flight_id: flight.id, passengers, cabin_class: flight.cabin_class }),
      });
    ...
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-box">
        ...
        <div className="modal-details">
          <div className="modal-row"><span>Flight</span><strong>{flight.flight_number}</strong></div>
          <div className="modal-row"><span>Depart</span><strong>{new Date(flight.departure_time).toLocaleString([], { dateStyle:"medium", timeStyle:"short" })}</strong></div>
          <div className="modal-row"><span>Arrive</span><strong>{new Date(flight.arrival_time).toLocaleString([], { dateStyle:"medium", timeStyle:"short" })}</strong></div>
          <div className="modal-row"><span>Cabin</span><strong>{flight.cabin_class}</strong></div>
          <div className="modal-row"><span>Passengers</span><strong>{passengers}</strong></div>
          <div className="modal-row modal-total"><span>Total</span><strong>${(Number(flight.base_price) * passengers).toLocaleString()}</strong></div>
        </div>
        ...
        {status?.type === "success" ? (
          <div className="modal-success">
            ...
            <p className="ms-ref">Reference: <strong>{status.ref}</strong></p>
            <button className="auth-btn" onClick={() => navigate("/profile/user")}>View My Bookings</button>
          </div>
        ) : (
          <>
            {status?.type === "error" && <div className="auth-error" style={{ marginBottom:"1rem" }}>{status.msg}</div>}
            <button className="auth-btn" onClick={confirmBooking} disabled={loading}>
              {loading ? <span className="auth-spinner" /> : `Book for $${(Number(flight.base_price) * passengers).toLocaleString()}`}
            </button>
            {!token && <p className="modal-hint">You'll be redirected to sign in first.</p>}
          </>
        )}
      </div>
    </div>
...
// ── FLIGHT RESULTS ─────────────────────────────────────────────────────────────
function FlightResults({ results, loading, passengers }) {
  ...
  return (
    <>
      {selected && <BookingModal flight={selected} passengers={passengers} onClose={() => setSelected(null)} />}
      <div className="results-wrap">
          ...
          {results.map((f) => (
            <div className="result-card" key={f.id}>
              ...
              <div className="rc-price">
                ...
                <button className="rc-btn" onClick={() => setSelected(f)}>Select</button>
              </div>
            </div>
          ))}
        </div>
    ...
    <main>
      ...
      <section className="hero" id="home">
        ...
        <div className="hero-content">
          ...
          <p className="hero-sub">Discover seamless journeys, exclusive offers, and world-class service crafted for the discerning traveller.</p>
        </div>
      ...
      {/* RESULTS */}
      <div id="results"><FlightResults results={results} loading={searching} passengers={passengers} /></div>
